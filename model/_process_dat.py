...
def main(datPath, scenPath, coi):
    ...
    df_uk.to_csv(os.path.join(scenPath, "df_uk.csv"))
    df_os.to_csv(os.path.join(scenPath, "df_os.csv"))
    xdf.to_csv(os.path.join(scenPath, "xdf.csv"))
    
    if "Item" not in kdf.columns:
        
        kdf.columns = [_ if _ != "level_0" else "Item" for _ in kdf.columns]
    
    kdf.to_csv(os.path.join(scenPath, "kdf.csv"))
