# -*- coding: utf-8 -*-
"""
Created on Mon Jan 27 09:26:07 2025
...
figsize = (8,7)
alpha = 0.8
ylabel = u"Extinction opporunity cost distribution (log10 $\Delta$E per functional-unit)"
# =============================================================================
...
functional_units = { "100 grams protein" : {"Ruminant meat" :0.26,
                  ...
                  "1000 kcal"       : {"Grains" : 3.5,
                                       "Rice" : 1.3,
                                       "Roots and tubers" : 0.87,
                                       },
                  
                  "100 grams"       : {"Vegetables" : 1,
                                       "Bananas" : 1,
                                       "Tropical fruit" : 1,
                                       "Temperate fruit" : 1,
                  ...
                  "1 serving"       : {"Coffee" : 1/0.15,
                                       "Cocoa" : 1/0.50,
                                       "Tea and maté" : 1/0.03,
                                       "Spices" : 1/0.15,
                                       },
                  
                  "1 kg\n of product"         : {"Oilcrops" : 1,
                                               "Sugar beet" : 0.16,
                                               "Sugar cane" : 0.15,}
...
colours_stim = colourdict[GROUPING]
...
for f, (fu, fu_dat) in enumerate(functional_units.items()):
    ...
    xdf = df[df[AGGREGATE].isin(fu_dat.keys())]
    xdf = xdf.sort_values("MQ", ascending = False)
    odf = pd.DataFrame()
    
    xdf = xdf[xdf.Group != "Other"]
    
    i = 0
    for item in xdf[AGGREGATE].unique():       
        # FILTER
        ...
        # PLOT
        ax.bar(xi + i, pr, bottom = LQ, fill=True, color = color, 
               edgecolor = color, alpha = alpha)
        # ax.scatter(i, p50, marker = "x", color = xcolor)
        ax.bar(xi + i, 0, bottom = MQ, fill=False, edgecolor = invert_color(color), alpha = 1)
    ...
    def addtext(ax,x,s):
        ax.text(x, -7.9, s,  fontsize = 13, horizontalalignment = "right",
                verticalalignment = "bottom",
                alpha = 0.8)
    
    if f != len(functional_units) - 1:
        ax.axvline(xi-1, color = "k", alpha = 0.7, linewidth = 1)
        tick_labels.append(None)
        addtext(ax, xi - 1.2, fu)
        
        
    else:
        addtext(ax, xi - 0.4, fu)
